      ...
      - 権限が管理者ではない場合、ユーザーの権限及びアクセス権限をチェックする
          ...
          - 課金ファイルに対して
              ...
              - コンテンツのアクセスを「オープンアクセス日を指定する」とした場合、以下のように制限する  
                ・オープンアクセス日が経過した場合、<Download>ボタンを表示する。  
                ・オープンアクセス日が経過していない場合、<Download>ボタンを表示しない。課金に設定されているロールを持つユーザの場合、「Charge」ボタンを表示する。
            
              - コンテンツのアクセスが「ログインユーザのみ」とした場合、  
                ・設定されているロールに所属するユーザーには、<Download>ボタンを表示する。  
                ・設定されているロールに所属しないユーザーには、<Download>ボタンを表示しない。
      ...
      - 課金ファイルに対して
          ...
          - コンテンツのアクセスが「オープンアクセス」以外とした場合、ファイルの情報を取得し、追加で以下の制限を実施する
              ...
              - アイテム詳細画面及び課金ファイルのリンクを押下したときに、ダウンロード可能かを判断する。
                  - 未ログインの場合：<Download>ボタン、「Charge」ボタンを表示しない。 
                  - 設定されていないロールの場合：「Download」ボタン、「Charge」ボタンを表示しない。
                  - 設定されているロールの場合：「Charge」ボタンを表示する。押下した場合、ポップアップで「￥XXXX (価格) かかります。購入しますか？」が表示される。
