...
(1)初期表示設定(Main Screen Initial Display Setting )

  - ［検索画面表示設定］タブの初期表示画面設定：「インデックスの検索結果を表示する(Index search result)」「ランキングを表示する(Ranking)」「コミュニティを表示する(Communities)」の中から選択し設定する。設定内容は以下のようになる。
      ...
      - 「WEBコンテンツを表示する(Web content)」：この項目を選択すると、「WEBコンテンツ表示設定」が表示され、表示可能な「Free Description」のウィジェットの一覧が表示される。初期画面中央部に選択されたウィジェットを表示する。
...
(2) インデックスツリー／ファセット検索設定

  - ［検索画面表示設定］タブのインデックスツリー／ファセット表示設定：インデックスツリーの表示有無と、表示時の横幅と縦幅を設定する。
...
(3) 使用可能な詳細検索条件の設定について
  ...
  - ［詳細検索条件設定］タブの詳細検索条件設定（Detail Search Conditions Setting）  
      ...
      - 表示内容及び設定内容は以下の通りである。
          ...
          - 初期選択（Initial Condition）  
            詳細検索の初期画面で表示する項目を設定する。
              ...
              - 使用項目をチェックしないとチェックボックスはチェックできない。
          ...
          - マッピング設定（Mapping setting）  
            詳細検索条件に指定できないメタデータの検索を行う際に使用する。ESにマッピングされているメタデータに関連づいたアイテムの検索を可能としている。  
            検索したいメタデータ名をテキスト（text）1\~10、integer\_JA\_1\~5、float\_JA\_1\~5、date\_JA\_1\~5、geopoint\_JA\_1、geoshape\_JA\_1のいずれかにあらかじめ設定しておき、該当行のマッピング設定ボタンを押下する。
            マッピング設定ボタン押下時に、マッピング設定画面をモーダル表示する。
            マッピング設定画面では、アイテムタイプをプルダウン形式で選択できる。その後、検索したいマッピングの指定についての条件を画面中央下部に設定し\[設全てのタブの情報を保存する]ボタン押下で設定する。
            
              - メタデータ名の設定は検索したいメタデータの検索方式に応じて使い分ける。  
                テキスト（text）1\~10：文字列での検索が可能。形式はjson。  
                integer\_JA\_1\~5：整数値での範囲検索が可能。形式はjson。  
                float\_JA\_1\~5：小数値での範囲検索が可能。形式はjson。  
                date\_JA\_1\~5：日付での範囲検索が可能。形式はjson。  
                geopoint\_JA\_1：緯度，経度を指定して検索が可能。形式はjson。  
                geoshape\_JA\_1：タイプと座標を指定して検索が可能。形式はjson。

              - ［マッピングパス］に当該検索項目として検索したいメタデータ要素をjson path を利用して指定する。
              - json path で値が検索項目が特定できない場合、条件となるメタデータ要素との比較で検索対象を特定する事が可能。
                  - ［条件］に条件となるメタデータ要素のjson path を指定する。
                  - ［条件値］に設定された条件となる比較対象の値を指定する。
                  - ［条件］のjson pathで指定されたメタデータ要素が［条件値］と一致した場合、［マッピングパス］に指定されたメタデータ要素が検索対象となる。
            
              - 上記の項目を設定し\[全てのタブの情報を保存する\]ボタン押下で、入力内容がdb内のsearch\_managementテーブルのsearch\_conditionsの内容が変更され、検索に反映される。
            
              - 要求された入力形式以外の文字列も入力し\[全てのタブの情報を保存する\]ボタンを押すことは可能であるが、入力形式に誤りがある場合は、db内のsearch\_managementテーブルのsearch\_conditionsは変更されず、検索結果にも影響を及ぼさない。

(4) 検索結果表示設定

  - ［検索結果設定］タブの検索結果表示設定（Search Results Setting）  
    デフォルトで表示する検索結果表示について設定する。
...
(5)著者検索設定
  ...
  - ［著者検索設定］タブの著者検索設定（Search Author Setting）  
