...
- position項目
  ...
  - 初期入力方法：select
  - 初期表示設定：True
  - 初期オプション：WEKO_USERPROFILES_POSITION_LIST[1:]（役職の選択肢）
- item1項目
  - 初期ラベル名：役職（その他）
  - 初期入力方法：position(other)
  - 初期表示設定：True
  - 初期オプション：空文字
- item2項目
  - 初期ラベル名：電話番号
...
- 初期入力方式は item1=`position(other)`、item2=`phonenumber`。position の初期オプションは `WEKO_USERPROFILES_POSITION_LIST[1:]`。item13〜item16 の初期表示フラグは `False`。university の初期ラベルは「大学・機関名」。
- メニュー自体は `WEKO_USERPROFILES_CUSTOMIZE_ENABLED`（既定 **False**、`ext.py`）で表示制御される。関連モジュール：weko-admin（画面）/ weko-user-profiles（config・消費側）。本機能は [制限公開 RESTRICTED_ACCESS_05](../restricted_access/RESTRICTED_ACCESS_05.md) と重複する。
