...
| isSupplementTo   | ～を補足している         |        | ○※2  |
| isIdenticalTo    | ～と同一である           |        | ○     |
| isDerivedFrom    | ～に由来している         |        | ○     |
| isSourceOf       | ～の由来になっている     |        | ○     |
| isCitedBy        | ～によって引用されている |        | ○※   |
| Cites            | ～を引用している         |        | ○※   |

※JPCOAR2.0で追加（https://schema.irdb.nii.ac.jp/ja/schema/2.0/20）

※2 isSupplementedByあるいはisSupplementToが選択されたとき、片方からのアイテムリンクが設定されると、逆方向のアイテムリンクは自動的に設定される。
...
- weko_workflow
- weko_records：「get_item_link_info」メソッドで、既存のアイテム間リンクの情報を取得する
- weko_search_ui：インデックスツリーからインデックスを選択したときに、「search」メソッドが呼び出される。「search」メソッドの中で「get_item_link_info」メソッドを呼び出している
...
「action_endpoint 'item_link'」の場合、以下の定義からアイテム間の関係性を取得、以下のテンプレートで取得した情報を表示する
