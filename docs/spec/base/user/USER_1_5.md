...
| 3 | count | 表示する検索結果件数を指定する。上限（100）を超える値が指定された場合、または0以下の値が指定された場合は、デフォルトで設定されている表示件数（20）にリセットして処理する。設定されない場合も同様にデフォルトの表示件数として処理する。 | 20 |
...
- `count` が上限（100）超過または負の場合は既定値 20 にリセットされる。
