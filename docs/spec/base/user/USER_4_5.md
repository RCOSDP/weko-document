...
- ファイル登録を以下のように実施する
  - ファイル登録
    ...
    - ファイルを追加した後、以下のようなファイルの情報を表示する
      ...
      - Progress：空白
      - Actions：ごみ箱ボタン
    - ファイル追加時に全文テキスト抽出対象（WEKO_MIMETYPE_WHITELIST_FOR_ES）の場合は抽出処理を行う。 抽出後の全文テキストサイズはWEKO_DEPOSIT_FILESIZE_LIMIT以下となる。全文テキストとメタデータ合わせてElasticsearchのhttp.max_content_length（デフォルトは100MB）以下である必要がある。
  ...
  - ファイル差し替え
    - インポート機能による本文ファイルの差し替えの場合  
      ...
      .metadata.item_files [xx] .filename を書き換え  
  ...
  - マルチパートファイルアップロード
    ...
    - アップロードが完了すると、アップロードしたファイル名を「ファイル名」に自動設定する  
      ファイルが複数の場合は、ファイル情報入力エリアをその数分追加した上でそれぞれ設定する  
      ［Start upload］ボタンは非活性化する
    - 「Resume」ボタン押下し、ファイルを再選択すると、失敗したファイルアップロードを再開できる。再開できる期間には、限度があり、 定数「FILES_REST_MULTIPART_EXPIRES」で定義される。この定数は、ObjectStorageのライフサイクル期間の日付と合わせる必要がある。
  - ファイル情報の入力
    - コンテンツファイルについて
      - 「表示名」（FileName）
      - 「本文URL」（Text URL）
...
- 設定キー： 「WEKO_RECORDS_UI_LICENSE_DICT」
- 現在の設定値：
