...
>アイテム編集可能なユーザ権限を所有している場合、下記手順48~54以外にも、直接URL入力することでアイテム編集画面に遷移できる。
>編集可能なユーザ且つ、当該アイテムが編集中ではない場合に編集画面に遷移でき、編集中の場合は図 6‑4のエラーを表示する。
>権限を所有していない場合はログイン画面を表示する。

表 5-27［アイテム編集用URL機能］の項目と説明

| 項番 | 項目      | 説明                                             | エラーメッセージ   |
| -- | ------- | ---------------------------------------------- | ---------------------------------------------- |
| 1  | アイテム編集中判定（セッション）     | 当該アイテムの編集中情報がセッション内に存在しているかどうか判定する | This Item is being edited.  |
| 2  | アイテム検索 | アイテム情報の検索を実行し、検索結果を判定する | Record does not exist.  |
| 3  | 権限判定   | アイテムを編集可能かどうか判定する | You are not allowed to edit this item.  |
| 4  | アイテムタイプ検索     | 論理削除されていないアイテムタイプが存在するか、item_typeテーブルの検索を実行し、検索結果を判定する | You do not even have an ItemType.  |
| 5  | 当該アイテムのアイテムタイプ検索     | 当該アイテムに紐付くアイテムタイプが存在するか、item_typeテーブルの検索を実行し、検索結果を判定する  | Dependency ItemType not found.  |
| 6  | 当該アイテムのインポート実行状態判定     | 当該アイテムがインポート実行中かどうか判定する。 | Item cannot be edited because the import is in progress.  |
| 7  | アクティビティ有無判定     | 当該アイテムのアイテムIDを利用して、workflow_activityテーブルの検索を実行し、検索結果を判定する   | 有なら 8へ <br> 無なら 9へ |
| 8  | アイテム編集中判定（DB）  | 7.で取得したアクティビティの「ステータス」の値から、アイテムが編集中かどうか判定する   | This Item is being edited.  |
| 9  | ワークフロー設定有無判定  | 当該アイテムのname_id、アイテムタイプのIDを利用して、workflow_workflowテーブルの検索を実行し、検索結果を判定する   | Workflow setting does not exist. |
...
編集されたアイテムがOAアシストから登録された場合、ファイルの登録・更新、公開ステータス、DOIの付与や取り下げのステータスを連携する。
...
削除されたアイテムがOAアシストから登録された場合、ファイルの削除・アイテム間連携の削除のステータスを連携する。
