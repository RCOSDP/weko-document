...
[16.11.1 初期表示を設定する 337](#初期表示を設定する)

[16.11.2 インデックスツリー/ファセットの表示を設定する 337](#インデックスツリー/ファセットの表示を設定する)

[16.11.3 詳細検索時の表示項目を設定する 338](#詳細検索時の表示項目を設定する)

[16.11.4 詳細検索のラベルを変更する 340](#詳細検索のラベルを変更する)

[16.11.5 詳細検索をカスタマイズする 342](#詳細検索をカスタマイズする)

[16.11.6 検索結果の表示内容を設定する 347](#検索結果の表示内容を設定する)

[16.11.7 著者検索を設定する 348](#著者検索を設定する)
...
この画面は機能により、4つのタブに分けて表示されています。
各タブの情報は[全てのタブの情報を保存する] をクリックすると全て保存されます。

#### 初期表示を設定する

トップページ画面初期表示時にメインコンテンツ内に表示する内容を設定することができます。初期表示を設定する方法を説明します。

1.  ［検索画面表示設定］タブの［初期表示設定］で、項目を設定します。
...
1.  ［検索画面表示設定］タブの［インデックスツリー/ファセット 表示設定］で項目を設定します。
    
    設定項目を次に示します。

![グラフィカル ユーザー インターフェイス, アプリケーション 自動的に生成された説明](media/media/image399.png)

表 16‑2インデックスツリー/ファセット表示の設定項目
...
1.  ［詳細検索条件設定］の［使用項目］と［初期選択］で、項目を選択します。
    ...
    ［使用項目］を選択しないと［初期選択］は選択できません。

![グラフィカル ユーザー インターフェイス, テキスト, アプリケーション 自動的に生成された説明](media/media/image393.png)

311. ［全てのタブの情報を保存する］をクリックします。
     
     設定が保存されます。

※詳細検索条件設定エリアの「JPCOARマッピング」の列は一例の文字列であり、
実際に定義されているJPCOARマッピングとの関係はありません。マッピング情報が多い場合は、後半部分を省略して表示します。
省略された内容を確認したい場合は、カーソルを合わせるとツールチップで全体が表示されます。

![グラフィカル ユーザー インターフェイス, テキスト, アプリケーション 自動的に生成された説明](media/media/image396.png)
...
［詳細検索条件設定］タブ内の検索条件の\[編集\]をクリックすると、詳細検索機能で表示される項目のラベルを変更することができます。

1.  ［編集］をクリックします。

![グラフィカル ユーザー インターフェイス 自動的に生成された説明](media/media/image394.png)

312. ラベルを編集し［設定（全てのタブの情報を保存ボタンを押さないと保存されません）］をクリックします。

![グラフィカル ユーザー インターフェイス 自動的に生成された説明](media/media/image395.png)

313. ［全てのタブの情報を保存する］をクリックします。
...
特定項目を詳細検索に設定するには［汎用詳細検索項目］に検索内容をマッピングします。
...
※表示項目の設定については「16.11.3 詳細検索時の表示項目を設定する」を参照してください。

※ラベルの設定については「16.11.4 詳細検索のラベルを変更する」を参照してください。

1.  詳細検索を設定したい［汎用詳細検索項目］の行の［マッピング設定］をクリックします。  
※ 添付の画面ではテキスト1～テキスト10の［汎用詳細検索項目］に詳細検索が設定されています。

![グラフィカル ユーザー インターフェイス, アプリケーション 自動的に生成された説明](media/media/image397.png)

314. ［アイテムタイプリスト］から検索対象とするアイテムタイプを選択します。

315. 当該検索項目として検索したいメタデータ要素をjson path を利用して［マッピングパス］に指定します。  
     （json pathの設定例：$.item\_1592405734122.attribute\_value\_mlt\[\*\].subitem\_1592369405220）  
     json path で値が検索項目が特定できない場合、条件となるメタデータ要素との比較で検索対象を特定する事が可能です。  
     ［条件］に条件となるメタデータ要素のjson path を指定します。  
     ［条件値］に設定された条件となる比較対象の値を指定します。  
     ［条件］のjson pathで指定されたメタデータ要素が［条件値］と一致した場合、［マッピングパス］に指定されたメタデータ要素が検索対象となります。  
...
316. 値編集し［設定（全てのタブの情報を保存ボタンを押さないと保存されません）］をクリックします。

317. ［全てのタブの情報を保存する］をクリックします。
...
1.  ［検索結果設定］タブの検索結果設定［表示結果表示設定］で、項目を設定します。
...
1.  ［検索結果設定］タブの［著者検索設定］で、項目を設定します。
...
<p>デフォルトは「著者名で検索」（チェックあり）です。</p></td>
</tr>
</tbody>
</table>

319. ［全てのタブの情報を保存する］をクリックします。
